	...
	public void finishCheck() {
		...
		if (cache.isEmpty()) {
			Minecraft.getMinecraft().thePlayer
					.addChatComponentMessage(new ChatComponentText("§a[CocoaUI]§c来自服务器的所有图片已经检查完毕。异常均已修正。"));
		} else {
			Minecraft.getMinecraft().thePlayer
					.addChatComponentMessage(new ChatComponentText("§a[CocoaUI]§c来自服务器的所有图片已经检查完毕。以下图片异常："));
			...
			Minecraft.getMinecraft().thePlayer
					.addChatComponentMessage(new ChatComponentText("无法正常加载，可能会影响到您的体验。请检查网络或联系服务器管理员."));
